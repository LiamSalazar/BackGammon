...
int bloqueoFicha(Elem destino, int jugador) {
    if (destino->cantidad >= 5 || destino == NULL) return 0;
    else
...
int obtenerPosicionOrigen(int jugador) {
    ...
    while (1) {
        ...
        if (bloqueoFicha(obtenerCasilla(&tablero, posicion), jugador)) {
            printf("Casilla bloqueada. Seleccione otra.\n");
            continue;
        }
        return posicion;
    }
...
void sacarFichaDeBarra(int jugador, int dado) {
    ...
    if (bar->cantidad > 0) {
        ...
        // Comer ficha si es posible
        if (destino->jugador != jugador && destino->cantidad == 1) {
            comerFicha(bar, destino, jugador);
            return;  
        }
        // Verificar si la casilla destino está bloqueada
